...
// invalid_macro_export_arguments - nightly
...
#[rustversion::nightly]
standard_lint!(undefined_naked_function_abi);
#[rustversion::nightly]
...
//
// Any clippy:: lint marked as `rustversion::since(1.44.1)` may have existed earlier, too. Leaving
// them as `since(1.44.1)` here, even though we support only from 1.45.
...
// `allow_prefixed` and `allow` crates don't support attribute parameters.)
//
